...
Endpoints UI:
  ...
  GET /autor  → perfil del autor y donaciones
...
<header>
  ...
  <div style="display:flex;align-items:center;gap:12px">
    ...
    <nav class="nav-links">""" + _nav("dashboard") + r"""</nav>
  </div>
</header>
<main>
<!-- KPIs -->
...
const REGION_COLORS={'latam':'#34d399','multipaís':'#a78bfa','mena':'#fbbf24','africa':'#f87171','asia':'#60a5fa','otros':'#fb923c','América Latina':'#34d399','Multipaís/Global':'#a78bfa','MENA':'#fbbf24','África':'#f87171'};
let _entidades=[],_fondos=[],_mensualData={},_chartMensual=null;
function mkChart(id,type,labels,datasets,opts={}){
  const ctx=document.getElementById(id);
  if(!ctx)return null;
  if(ctx._chart)ctx._chart.destroy();
  const c=new Chart(ctx,{type,data:{labels,datasets},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#a0aec0',font:{size:11}}}},scales:type!=='pie'&&type!=='doughnut'?{x:{ticks:{color:'#7c8db5',font:{size:10}},grid:{color:'#1e2235'}},y:{ticks:{color:'#7c8db5',font:{size:10}},grid:{color:'#1e2235'}}}:{},...opts}});
...
async function cargar(){
  ...
  ].map(r=>`<div class="bar-wrap"><div class="bar-label"><span>${r.lbl}</span><span>${r.pct}%</span></div><div class="bar-bg"><div class="bar-fill ${r.cls}" style="width:${r.pct}%"></div></div></div>`).join('');
  ...
  mkChart('chartEslabones','bar',Object.keys(dist).sort().map(e=>`E${e}`),
    [{label:'Fondos por eslabón',data:Object.keys(dist).sort().map(e=>dist[e]),backgroundColor:COLORS,borderRadius:4}],{plugins:{legend:{display:false}}});
  const acum=res.acumulativo_anual||[];
  mkChart('chartAnual','bar',acum.map(a=>String(a.año)),[
    {label:'Importe año (M€)',data:acum.map(a=>+(a.importe/1e6).toFixed(1)),backgroundColor:'#3b82f6',borderRadius:4,yAxisID:'y'},
    {label:'Acumulado (M€)',data:acum.map(a=>+(a.importe_acum/1e6).toFixed(1)),type:'line',borderColor:'#34d399',borderWidth:2,pointRadius:4,backgroundColor:'transparent',yAxisID:'y1'},
  ],{scales:{y:{position:'left',ticks:{color:'#7c8db5',font:{size:10}},grid:{color:'#1e2235'}},y1:{position:'right',ticks:{color:'#34d399',font:{size:10}},grid:{display:false}},x:{ticks:{color:'#7c8db5'},grid:{color:'#1e2235'}}},plugins:{legend:{labels:{color:'#a0aec0',font:{size:10}}}}});
  const topPaises=res.top_paises||[];
  const regiones={};
  topPaises.forEach(p=>{let reg='Otros';const n=p.pais_region||'';if(['Bolivia','Colombia','Ecuador','Guatemala','Honduras','México','Nicaragua','Perú','Cuba','Haití','Venezuela','El Salvador','Costa Rica','Panamá','Paraguay','Brasil','Chile','Argentina'].some(x=>n.includes(x)))reg='América Latina';else if(['Marruecos','Túnez','Argelia','Jordania','Líbano','Palestina','Siria','Irak','Yemen','Mediterráneo'].some(x=>n.includes(x)))reg='MENA';else if(['Etiopía','Mozambique','Mali','Niger','Senegal','Chad','Kenya','Tanzania','Uganda','Ghana','Mauritania','Burkina','África'].some(x=>n.includes(x)))reg='África';else if(['Global','Especificado','Multipaís','América Latina y Caribe'].some(x=>n.includes(x)))reg='Multipaís/Global';regiones[reg]=(regiones[reg]||0)+p.importe;});
  ...
  document.getElementById('tabsRegion').innerHTML=regiones_mens.map((r,i)=>`<div class="tab ${i===0?'active':''}" onclick="switchRegion('${r}',this)">${r}</div>`).join('');
...
<header>
  ...
  <nav class="nav-links">""" + _nav("manual") + """</nav>
</header>
<main>
<div class="card">
...
<div class="card">
  ...
  <p>Fondos transferidos a Organismos Internacionales (PNUD, UNICEF, FAO, ACNUR…) que agregan contribuciones multi-donante sin desglosar la aportación española en el estándar IATI.</p>
  <h3><span class="badge r2">R2</span> Sub-contratación sin OCDS</h3>
  <p>Contratos adjudicados directamente o sin publicación en el Portal de la Contratación del Estado (PLACE) bajo el estándar Open Contracting Data Standard.</p>
...
# ─────────────────────────────────────────────────────────────────────────────
# AUTOR HTML  ← NUEVA PÁGINA
# ─────────────────────────────────────────────────────────────────────────────
AUTOR_HTML = """<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
.autor-avatar{
  ...
  background:linear-gradient(135deg,#2d3561,#3b82f6);
...
.email-btn{
  ...
  background:#2d3561;color:#7eb8f7;
  padding:7px 16px;border-radius:8px;font-size:0.83rem;
  text-decoration:none;transition:background .2s;border:1px solid #3b52a0;
}
.email-btn:hover{background:#3b52a0;text-decoration:none}
...
.dona-row .val.mono{
  font-family:monospace;color:#7eb8f7;font-size:0.9rem;
  background:#0f1117;padding:3px 8px;border-radius:5px;display:inline-block;margin-top:2px;
}
...
<header>
  <div>
    ...
    <span class="sub">Monitor Trazabilidad AECID — Fenómenos Corruptivos</span>
  </div>
  <nav class="nav-links">""" + _nav("autor") + """</nav>
</header>
<main>
  ...
  <div class="autor-card">
    ...
    <div class="autor-info">
      ...
      <p>
        ...
        <em>XAI</em> aplicado al análisis de contrataciones públicas.
      </p>
      <p class="pub">
        Publicaciones en <em>Journal of Financial Crime</em> (Emerald Publishing).
  ...
  <div class="dona-grid">
    ...
    <div class="dona-card wire">
      ...
      <div class="dona-row"><div class="lbl">CUIT</div><div class="val mono">20-12034411-1</div></div>
    </div>
